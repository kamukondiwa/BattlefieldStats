...
popup.accordion = function() {
	...
	$('.accordion .title').click(function() {
		$(this).parent().find('.box.open').addClass('sliding').slideUp(530).addClass('closed').removeClass('open').removeClass('sliding');

		var content = $(this).next();

		if(content.is(':visible')) {
			content.addClass('sliding').slideUp(530).addClass('closed').removeClass('open').removeClass('sliding');
		} else {
			$(this).parent().find('.box.open').addClass('sliding').slideUp(530).addClass('closed').removeClass('open').removeClass('sliding');

			content.addClass('sliding').slideDown(500).removeClass('closed').addClass('open').removeClass('sliding');
		}
	});
};
...
popup.unlocks = function(data) {
	var upcomingUnlocks = [];
	var weapons = $.Enumerable.FromProps(data.stats.weapons).ToArray();
	...
	$('.previous').click(function() {
		if(popup.context.page > 1) {
			$('.page.active').hide('slide', {
				...
				$(this).prev().find('.box').hide();
				$(this).prev().show('slide', {
					direction: 'left'
				}, 530, function() {
					popup.context.page -= 1;
					popup.updateNavigation();
...
popup.requestPlayer = function(request, response) {
	...
	$.ajax({
		crossDomain: true,
		type: 'POST',
		url: 'http://api.bf3stats.com/' + popup.context.platform + '/player/',
		data: {
			player: request.term
		},
